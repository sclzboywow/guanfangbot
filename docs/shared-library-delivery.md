...
1. 用户发送 `@机器人 不动产`。
2. 机器人检索标题，优先返回当前网盘仍可定位的哈希路径文件，并刷新过期 `fsid`；返回总数和前 5 个可分享结果。
3. 同一用户在同一群内直接回复 `1` 到 `5`，不需要再次 @机器人。
4. 后端使用所选记录的 `fsid`（必要时按网盘路径重解析）创建百度网盘分享，并发送标题、分享链接、4 位提取码和有效期。

检索结果与发货消息使用多行排版；其他提示仍保持简洁。检索会话默认 3 分钟有效，按机器人、群和用户隔离，成功发货后立即失效。
...
method：set（兼容旧配置 rapidshare，会自动映射为 set）
有效期：7 天
