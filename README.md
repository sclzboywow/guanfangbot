...
优先推荐使用“官方群管理”的入群申请审批与自动审批策略，让不符合条件的用户无法入群。“官方群管理 → 兼容验证”用于已经入群后仍需验证的场景，并提供两种可直接填写的方式：

1. **数学题验证**：用户无需 @ 机器人，直接发送纯数字答案；错误答案、普通聊天、图片或文件会被撤回。管理员可填写验证时限、最大错误次数，并选择失败后“官方禁言”或“继续仅撤回”。
2. **人工审核**：收到 `GROUP_MEMBER_ADD` 后立即使用QQ官方接口禁言，管理员在开发台点击通过后解除验证禁言。

验证超时由后台定时检查，也会在状态页刷新和成员再次发言时补充处理。升级前已经存在的待验证记录不会被批量补禁言，避免部署后突然影响存量成员。

验证禁言、广告治理禁言和管理台人工禁言分别记录；解除其中一种原因时，会保留其他仍生效的禁言，防止“验证通过”误解除治理处罚。所有机器人发送到群里的题目、通过提示和管理员操作提示都会压缩为单行文本。
