...
async function save() {
  ...
  try {
    const payload = {
      ...
      merged_message_action: settings.value.merged_message_action,
      group_card_action: settings.value.group_card_action,
      special_rule_mute_minutes: Number(settings.value.special_rule_mute_minutes),
        ...
        <div class="layout">
          <div class="main-column">
            <section class="card panel">
              ...
              <div v-if="settings.retract_merged_messages || settings.retract_group_cards" class="special-actions">
                <div v-if="settings.retract_merged_messages" class="field">
                  <label>合并消息触发后</label>
                  <select v-model="settings.merged_message_action" class="select">
                    <option value="retract">仅撤回本条</option>
                    <option value="mute">撤回并使用官方禁言</option>
                  </select>
                </div>
                <div v-if="settings.retract_group_cards" class="field">
                  <label>群名片触发后</label>
                  <select v-model="settings.group_card_action" class="select">
                    <option value="retract">仅撤回本条</option>
                    <option value="mute">撤回并使用官方禁言</option>
                  </select>
                </div>
                <div v-if="settings.merged_message_action === 'mute' || settings.group_card_action === 'mute'" class="field">
                  <label>专项禁言时长（分钟）</label>
                  <input v-model.number="settings.special_rule_mute_minutes" class="input" type="number" min="1" max="43200">
                </div>
                <p class="special-hint">触发消息始终先撤回；选择禁言时，还需开启“使用QQ官方禁言”。该禁言与入群验证、人工禁言分别记录，解除验证不会提前解除治理禁言。</p>
              </div>
