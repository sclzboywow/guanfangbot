...
const mathEnabled = ref(true)
const customQuestionEnabled = ref(false)
const combinationMode = ref<'all' | 'random_one'>('all')
const customQuestion = ref('请回答本群的入群验证问题')
const customAnswersText = ref('')
const customIgnoreCase = ref(true)
...
async function loadStatus() {
  ...
  try {
    ...
    mathEnabled.value = result.settings.math_enabled
    customQuestionEnabled.value = result.settings.custom_question_enabled
    combinationMode.value = result.settings.combination_mode
    customQuestion.value = result.settings.custom_question
    customAnswersText.value = result.settings.custom_answers.join('\n')
    customIgnoreCase.value = result.settings.custom_ignore_case
    minOperand.value = result.settings.min_operand
    maxOperand.value = result.settings.max_operand
    timeoutSeconds.value = result.settings.timeout_seconds
...
async function saveSettings() {
  ...
  try {
    status.value = await api.updateGroupVerificationSettings(botId.value, {
      ...
      math_enabled: mathEnabled.value,
      custom_question_enabled: customQuestionEnabled.value,
      combination_mode: combinationMode.value,
      custom_question: customQuestion.value,
      custom_answers: [...new Set(customAnswersText.value.split(/[\n,，]+/).map(item => item.trim()).filter(Boolean))],
      custom_ignore_case: customIgnoreCase.value,
    ...
    message.value = enabled.value
      ? '入群后验证已启用；请确认 QQ 管理端已开通三个必需事件，并将机器人设为群管理员。'
...
async function runAction(session: GroupVerificationSession, action: 'verify' | 'reset' | 'close') {
  ...
  try {
    ...
    message.value = action === 'verify' ? '已手动通过' : action === 'reset' ? '已重新出题并发送单行提示' : '验证记录已结束'
  } catch (e) {
  ...
  <section class="page verification-page">
    <div class="page-head">
      <div>
        ...
        <p class="page-sub">入群前审批与入群后验证相互独立；数学题和自定义问题均可单独开关。</p>
      </div>
    ...
    <template v-else>
      ...
      <template v-else-if="status">
        <div class="summary-grid">
          ...
          <section class="card summary-card">
            <span>失败禁言</span>
            <strong class="warn">{{ status.counts.failed }}</strong>
            <small>达到错误上限或超时，使用QQ官方禁言</small>
          </section>
          <section class="card summary-card">
            <span>已结束</span>
            <strong>{{ status.counts.removed }}</strong>
            <small>离群或管理员手动结束</small>
          </section>
        </div>
        ...
        <div class="layout">
            ...
            <section class="card panel settings-panel">
              <div class="section-head">
                <div>
                  ...
                  <p class="section-sub">答题期间不预先禁言；错误消息撤回，超时或错误达上限后官方禁言。</p>
                </div>
              ...
              <div class="method-grid">
                <label class="method-card"><input v-model="mathEnabled" type="checkbox"><span><b>数学问题</b><small>随机加减法，答案必须是纯数字</small></span></label>
                <label class="method-card"><input v-model="customQuestionEnabled" type="checkbox"><span><b>自定义问题</b><small>配置问题和一个或多个正确答案</small></span></label>
              </div>

              <div v-if="mathEnabled && customQuestionEnabled" class="field combination-field"><label>同时开启时</label><select v-model="combinationMode" class="select"><option value="all">两种都答对</option><option value="random_one">随机选择一种</option></select></div>

              <div v-if="mathEnabled" class="number-grid">
                <div class="field">
                  <label>最小数字</label>
              ...
              <div v-if="customQuestionEnabled" class="custom-grid">
                <div class="field"><label>自定义问题</label><input v-model="customQuestion" class="input" maxlength="200"></div>
                <div class="field"><label>正确答案</label><textarea v-model="customAnswersText" class="textarea" rows="4" placeholder="每行一个答案"></textarea><small>可设置多个答案，每行一个。</small></div>
                <label class="switch-row inline-switch"><input v-model="customIgnoreCase" type="checkbox"><span>英文答案忽略大小写</span></label>
              </div>

              <div class="number-grid failure-grid">
                <div class="field"><label>验证超时（秒）</label><input v-model.number="timeoutSeconds" class="input" type="number" min="30" max="3600"></div>
                <div class="field"><label>最多错误次数</label><input v-model.number="maxWrongAttempts" class="input" type="number" min="1" max="20"></div>
                <div class="field"><label>失败禁言（分钟）</label><input v-model.number="failureMuteMinutes" class="input" type="number" min="1" max="43200"></div>
              </div>
              ...
              <div class="behavior-grid">
                <div><b>无需 @ 机器人</b><span>直接发送当前问题答案</span></div>
                <div><b>验证超时</b><span>{{ timeoutSeconds }} 秒后官方禁言</span></div>
                <div><b>错误消息撤回</b><span>文字、图片、文件等全部处理</span></div>
                <div><b>机器人消息单行</b><span>题目和成功提示均不换行</span></div>
              </div>

              <div class="preview">
                <span>群内提示示例</span>
                <code>{{ customQuestionEnabled && !mathEnabled ? `欢迎加入本群，请先完成验证：${customQuestion} 请直接发送答案文字。` : '欢迎加入本群，请先完成验证：8 + 7 = ? 请直接发送数字答案。' }}</code>
              </div>
              <div class="preview">
                <span>验证成功提示预览</span>
            ...
            <section class="card panel sessions-panel">
              <div class="section-head sessions-head">
                <div>
                  ...
                  <p class="section-sub">状态和撤回计数保存在 SQLite，容器重启后不会丢失。</p>
                </div>
                <div class="filters">
                ...
                <table>
                  ...
                  <tbody>
                    <tr v-for="session in filteredSessions" :key="session.id">
                      ...
                      <td><strong>{{ session.question }}</strong><small>{{ session.challenge_type === 'math' ? `答案 ${session.answer}` : `自定义答案 ${session.accepted_answers.join(' / ')}` }}</small></td>
                      <td><span class="state" :class="session.status">{{ statusText(session.status) }}</span><small v-if="session.failure_reason" class="row-error">{{ session.failure_reason === 'timeout' ? '验证超时' : '错误次数达到上限' }}</small><small v-if="session.last_error" class="row-error">{{ session.last_error }}</small></td>
                      <td><strong>{{ session.retracted_messages }}</strong><small>错误 {{ session.wrong_attempts }} 次</small></td>
                      <td><span>{{ formatTime(session.joined_at) }}</span><small v-if="session.last_message_at">最后消息 {{ formatTime(session.last_message_at) }}</small></td>
                      <td>
                        <div class="row-actions">
                          ...
                          <button class="mini" :disabled="actionId === session.id" @click="runAction(session, 'reset')">重新出题</button>
          ...
          <aside class="side-column">
            <section class="card panel requirement-panel">
              ...
              <div class="admin-warning"><strong>群管理员权限</strong><span>机器人必须在目标群内被设为管理员，才能撤回未验证成员的消息。</span></div>
            </section>
...
.success-message-field{margin-top:16px}.success-message-field small{display:block;margin-top:6px;color:var(--ink-4);font-size:10.5px}.state.failed{background:rgba(255,59,48,.1);color:var(--danger)}.method-grid{display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-top:18px}.method-card{display:flex;align-items:center;gap:10px;padding:13px;border:1px solid var(--line);border-radius:12px;background:var(--bg-sunken)}.method-card input{accent-color:var(--accent)}.method-card span,.method-card b,.method-card small{display:block}.method-card small{margin-top:4px;color:var(--ink-4);font-size:10px}.combination-field,.custom-grid{margin-top:16px}.custom-grid{display:grid;gap:12px}.textarea{width:100%;resize:vertical;padding:11px 12px;border:1px solid var(--line);border-radius:11px;background:white;color:var(--ink);font:inherit;line-height:1.5}.custom-grid small{display:block;margin-top:5px;color:var(--ink-4);font-size:9.5px}.inline-switch{width:max-content}.failure-grid{grid-template-columns:repeat(3,1fr)}@media(max-width:700px){.method-grid,.failure-grid{grid-template-columns:1fr}}
</style>
