...
const actionId = ref('')
const filter = ref<'all' | 'pending' | 'verified' | 'removed'>('pending')
const filters = [
  ...
  <section class="page verification-page">
    <div class="page-head">
      <div>
        ...
        <p class="page-sub">成员入群后直接回答简易数学题；未通过前的其他群消息会被管理员机器人自动撤回。</p>
      </div>
            ...
            <section class="card panel settings-panel">
              <div class="section-head">
                <div>
                  ...
                  <p class="section-sub">验证没有超时；成员一直保持待验证，直到答对、离群或管理员结束记录。</p>
                </div>
                ...
                <table>
                  ...
                  <tbody>
                    <tr v-for="session in filteredSessions" :key="session.id">
                      ...
                      <td><strong>{{ session.question }}</strong><small>答案 {{ session.answer }}</small></td>
                      <td><span class="state" :class="session.status">{{ statusText(session.status) }}</span><small v-if="session.last_error" class="row-error">{{ session.last_error }}</small></td>
                      ...
                      <td>
                        <div class="row-actions">
                          <button v-if="session.status === 'pending'" class="mini good-btn" :disabled="actionId === session.id" @click="runAction(session, 'verify')">通过</button>
