...
async function submit() {
  ...
  try {
    ...
    const redirect = typeof route.query.redirect === 'string' ? route.query.redirect : '/bots'
    await router.replace(redirect || '/bots')
  } catch (err) {
...
<template>
  <div class="auth-page">
    <form class="auth-card" @submit.prevent="submit">
      ...
      <label class="field">
        <span>密码</span>
        <input v-model="password" class="input" type="password" autocomplete="current-password" required minlength="8" />
      </label>
      <p v-if="error" class="error">{{ error }}</p>
      <button class="btn primary" type="submit" :disabled="submitting">{{ submitting ? '登录中…' : '登录' }}</button>
